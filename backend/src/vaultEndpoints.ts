...
/**
 * Submits a Soroban RPC call wrapped in the circuit breaker and a trace span.
 * Invokes the vault contract to deposit or withdraw funds.
 */
async function submitSorobanTx(type: string, payload: Record<string, unknown>): Promise<string> {
  return sorobanCircuitBreaker.execute(() =>
    withSpan('soroban.rpc.submit', async (span) => {
      ...
      span.setAttributes({
        'rpc.type': type,
        'rpc.wallet': walletAddress,
        'rpc.amount': amount,
        'rpc.asset': asset,
      });

      try {
        // Call the real Soroban contract
...
async function handleVaultOperation(
  ...
  try {
    const invalidateReadCaches = async () => invalidateCache();
    ...
    if (err instanceof SorobanSimulationError) {
      // Surface Soroban simulation/submission errors as 502 Bad Gateway
      logger.log('error', `Soroban simulation error for ${type}`, {
        error: err.message,
        code: err.code,
        traceId: getCurrentTraceId(),
      });
      return res.status(502).json({
        error: 'Bad Gateway',
        status: 502,
        message: `Soroban transaction failed: ${err.message}`,
        code: err.code,
      });
    }
